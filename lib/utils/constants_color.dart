import 'package:flutter/material.dart';
import '../model/data_controllers.dart';

const Color WHITE_COLOR = Color(0Xfffcfdfd);
const Color BOTTOM = Color(0xFF8EC5D4);
const Color BLACK_COLOR = Colors.black;
const Color BLUE_COLOR = Color(0xff1d2e3e);
const Color Gold = Color(0xfff4d354);
const Color GRAY = Color(0xffbdbdbd);
const Color BLUEGRAY = Color(0xff78909c);
const String CARD = 'assets/lottie/No_Data.json';
const String QR = 'assets/lottie/QR.json';
const YOUR_GOOGLE_API_KEY = 'AIzaSyDJbqVkRb6llQczHvPN_WnK4pGUGt30ykM';

class Staticmenbers {
  static List<Users> listofUsers = [];
  static List<Users> cardUsers = [];
}

final colorList = [
  Colors.blue,
  const Color(0xff081c1b),
  const Color(0xff56433d),
  const Color(0xffa82fed),
  const Color(0xff4A964E),
  const Color(0xff4c464e),
  const Color(0xff042f3a),
  const Color(0xff18313c),
  const Color(0xff3a414b),
  const Color(0xff0c3150),
  const Color(0xff960001)
];
